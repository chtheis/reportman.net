...
//				if (doc.PrinterSettings.ToPage > FMeta.Pages.CurrentCount)
		...
		protected void UpdateHardMargins()
		{
			UseHardMargin = true;
#if REPMAN_DOTNET2
			HardMarginX = (int)Math.Round(doc.DefaultPageSettings.HardMarginX * 1440 / 100);
			HardMarginY = (int)Math.Round(doc.DefaultPageSettings.HardMarginY * 1440 / 100);
#else
#if REPMAN_MONO
			// Mono 1.X
			HardMarginX = 0;
      ...
      finally
      {
		...
		PaperSize FindPaperSize(int index,int width,int height)
		{
			...
			if (BrowsePaperSizes)
			{
				...
				{
					current = nlist[i];
					if (Math.Abs(current.Width*10 - newsizex) <= DIFF_MARGIN)
						if (Math.Abs(current.Height*10 - newsizey) <= DIFF_MARGIN)
						{
							npapersize = current;
							found = true;
							break;
