...
namespace Reportman.Drawing
{
	...
	public class TotalPages
	{
		...
		{
			FCount = 0;
			FObjects = new TotalPage[FIRST_ALLOCATION_OBJECTS];
		}
		public void Clear()
		{
			FCount = 0;
		...
		private void CheckRange(int index)
		{
			if ((index < 0) || (index >= FCount))
				throw new Exception("Index out of range on TotalPagesList collection");
		}
		public TotalPage this[int index]
		{
			get { CheckRange(index); return FObjects[index]; }
			set { CheckRange(index); FObjects[index] = value; }
		}
		public int Count { get { return FCount; } }
		public void Add(TotalPage obj)
		{
			if (FCount > (FObjects.Length - 2))
			{
				TotalPage[] nobjects = new TotalPage[FCount];
				System.Array.Copy(FObjects, 0, nobjects, 0, FCount);
				FObjects = new TotalPage[FObjects.Length * 2];
	...
	public class MetaObjects
	{
		...
		{
			FCount = 0;
			FObjects = new MetaObject[FIRST_ALLOCATION_OBJECTS];
		}
		public void Clear()
		{
		...
		{
			if ((index < 0) || (index >= FCount))
				throw new Exception("Index out of range on MetaObject collection");
		}
		public MetaObject this[int index]
		{
			get { CheckRange(index); return FObjects[index]; }
			set { CheckRange(index); FObjects[index] = value; }
		}
		...
		public void Add(MetaObject obj)
		{
			if (FCount > (FObjects.Length - 2))
			{
				MetaObject[] nobjects = new MetaObject[FCount];
				System.Array.Copy(FObjects, 0, nobjects, 0, FCount);
				FObjects = new MetaObject[FObjects.Length * 2];
				System.Array.Copy(nobjects, 0, FObjects, 0, FCount);
			}
			FObjects[FCount] = obj;
			FCount++;
...
#else
	public class MetaObjects:List<MetaObject>
	{
