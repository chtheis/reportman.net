...
namespace Reportman.Drawing
{
	...
	public class PDFCanvas
	{
		...
		private FontInfoProvider FDefInfoProvider;
      ...
      OldBrushColor = -1;
			...
			Lines = new LineInfos();
#if PocketPC
                FDefInfoProvider = new FontInfoGDI();
#else
            if ((System.Environment.OSVersion.Platform == PlatformID.Unix) || (System.Environment.OSVersion.Platform == PlatformID.MacOSX))
                FDefInfoProvider = new FontInfoFt();
            else
                FDefInfoProvider = new FontInfoGDI();
#endif
        }
    ...
    public string PDFCompatibleTextWithKerning(string astring, TTFontData adata, PDFFont pdffont)
			...
			bool wordbreak, int Rotation, bool RightToLeft)
		{
			...
			try
			{
                ...
                if (Rotation != 0)
                {
                    ...
                    Rotate(rotrad);
                    arect = new Rectangle(0, 0, arect.Width-arect.Left, arect.Height-arect.Top);
                }
				...
				if (!Font.Transparent)
				{
                    ...
                    if ((Alignment & AlignmentFlags_AlignHCenter) > 0)
                        bleft = arect.Left + (arect.Width - arect.Left) / 2 - recsize.Width / 2;
                    else
                    {
                        if (   ( ((Alignment & AlignmentFlags_AlignRight) > 0) && (!RightToLeft))
                              || (((Alignment & AlignmentFlags_AlignRight) == 0) && (RightToLeft))
                               )
                            bleft = arect.Left + (arect.Width - arect.Left) - recsize.Width;
                        else
                            bleft = arect.Left;
                    }
                    if ((Alignment & AlignmentFlags_AlignVCenter) > 0)
                        btop = arect.Height + (arect.Height - arect.Top) / 2 - recsize.Height / 2;
                    else
                        if ((Alignment & AlignmentFlags_AlignBottom) > 0)
                            btop = arect.Top + (arect.Height-arect.Top) - recsize.Height;
                        else
                            btop = arect.Top;
                    Rectangle(bleft, btop, bleft+recsize.Width, btop+recsize.Height);
				}
				// Align bottom or center
				posy = arect.Top;
				if ((Alignment & AlignmentFlags_AlignBottom) > 0)
				{
					posy = arect.Height - recsize.Height;
				}
				if ((Alignment & AlignmentFlags_AlignVCenter) > 0)
				{
					posy = arect.Top + (int)(((arect.Height - arect.Top) - recsize.Height) / 2);
				}
				for (i = 0; i < Lines.Count; i++)
				{
					...
					if ((Alignment & AlignmentFlags_AlignRight) > 0)
					{
						...
						posx = arect.Width - Lines[i].Width;
					}
					// Aligns horz.
					if ((Alignment & AlignmentFlags_AlignHCenter) > 0)
					{
						posx = arect.Left + (int)(((arect.Width - arect.Left) - Lines[i].Width) / 2);
					}
					...
					if (((Alignment & AlignmentFlags_AlignHJustify) > 0) && (!Lines[i].LastLine))
					{
						...
						for (index = 0; index < lwords.Count; index++)
						{
							...
							int nwidth;
							if (RightToLeft)
								nwidth = -(arec.Width - arec.Left);
							else
								nwidth = arec.Width - arec.Left;
							lwidths.Add(nwidth);
							alinesize = alinesize + arec.Width - arec.Left;
						}
						alinedif = arect.Width - arect.Left - alinesize;
						if (alinedif > 0)
						{
							if (lwords.Count > 1)
								alinedif = (int)alinedif / (lwords.Count - 1);
							if (RightToLeft)
							{
								currpos = arect.Width;
								alinedif = -alinedif;
							...
							for (index = 0; index < lwords.Count; index++)
							{
								TextOut(currpos, posy + Lines[i].TopPos, lwords[index], Lines[i].Width, 0, RightToLeft);
			...
			bool tile, bool clip, long internal_imageindex)
		{
            ...
            try
            {
                ...
                bool isgif = false;
                isjpeg = GraphicUtils.GetJPegInfo(abitmap, out bitmapwidth, out bitmapheight);
                if (isjpeg)
                {
                ...
                {
                    abitmap.Seek(0, System.IO.SeekOrigin.Begin);
                    if (!GraphicUtils.GetBitmapInfo(abitmap, out bitmapwidth, out bitmapheight,
                        out imagesize, fimagestream, out indexed, out bitsperpixel, out numcolors, out palette, out isgif))
                    {
                        abitmap.Seek(0, System.IO.SeekOrigin.Begin);
                        System.Drawing.Image nimage = System.Drawing.Image.FromStream(abitmap);
                        abitmap = new MemoryStream();
                        nimage.Save(abitmap,System.Drawing.Imaging.ImageFormat.Bmp);
                        abitmap.Seek(0, System.IO.SeekOrigin.Begin);
                        GraphicUtils.GetBitmapInfo(abitmap, out bitmapwidth, out bitmapheight,
                        out imagesize, fimagestream, out indexed, out bitsperpixel, out numcolors, out palette, out isgif);                         
                    }
                    if (isgif)
                    {
                ...
                if (newstream)
                {
                    if (!File.CalculateOnly)
                    {
                        ...
                        StreamUtil.SWriteLine(astream, "/Length " + imagesize.ToString());
                        StreamUtil.SWriteLine(astream, "/Name /Im" + File.ImageCount.ToString());
                        if (isjpeg)
                        ...
                        {
                            StreamUtil.CompressStream(fimagestream, astream, File.OptimizeSize);
                        }
                        else
#endif
        ...
        public void TextExtent(string Text, ref Rectangle rect, bool wordbreak, bool singleline, bool dolineinfo)
        {
            ...
            int currenttoppos = 0;
            double recwidth = (double)(rect.Width - rect.Left) / FResolution * PDFFile.CONS_PDFRES;
            if (dolineinfo)
                Lines.Clear();
            // Replace cr/lf for only cfs
...
//                if (astring[i] == (char)13)
                ...
                if ((astring[i] == (char)10))
                {
                    ...
                    if (linfo.Size < 0)
                        linfo.Size = 0;
                    linfo.Width = (int)Math.Round((currentwidth / PDFFile.CONS_PDFRES * FResolution));
            ...
            Rectangle arec = new Rectangle(rect.Left, rect.Top, 
                                           (int)Math.Round((maxwidth / PDFFile.CONS_PDFRES * FResolution)),  
                                           totalheight);
            rect = arec;
        }
		...
		void SetFontType()
		{
			...
			for (i = 0; i < FCanvas.FontData.Count; i++)
			{
				...
				if (adata.Embedded)
				{
					...
					StreamUtil.SWriteLine(FTempStream, "<< /Length " + fontcontent.Length.ToString());
					...
					if (Compressed)
					{
						StreamUtil.CompressStream(fontcontent, FTempStream,OptimizeSize);
					}
					else
#endif
                        fontcontent.WriteTo(FTempStream);
 ...
 */
		...
		void EndStream()
		{
			...
			TempSize = FTempStream.Length - FStreamSize1 - FStreamSize2 - "Stream".Length - "endstream".Length - 6;
			FObjectCount = FObjectCount + 1;
			FTempStream.SetLength(0);
			StreamUtil.SWriteLine(FTempStream, FObjectCount.ToString() + " 0 obj");
		...
		void WriteBitmap(int index)
		{
            if (!CalculateOnly)
            {
                ...
                StreamUtil.SWriteLine(FTempStream, "" + (char)13 + (char)10 + "endstream");
                StreamUtil.SWriteLine(FTempStream, "endobj");
                AddToOffset(FTempStream.Length);
                FTempStream.WriteTo(FMainPDF);
            }
