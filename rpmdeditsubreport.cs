...
//      npicture.SetBounds(0, 0, npicture.Width, npicture.Height);
      ...
      MarginResize = Twips.TwipsToPixels(1440 * 7, FDrawScale);
      ...
      foreach (Section xsection in FSubReport.Sections)
      {
        ...
        ninfo.Height = Twips.TwipsToPixels(xsection.Height,FDrawScale);
        ninfo.Width = Twips.TwipsToPixels(xsection.Width, FDrawScale);
      ...
      ninfo.Height = 0;
      ninfo.Width = Twips.TwipsToPixels(LastSection.Width, FDrawScale);
      if (ninfo.Width > MaxWidth)
        MaxWidth = ninfo.Width;
              ...
              using (Graphics grsec = Graphics.FromImage(nband.SectionBitmap))
              {
                  ...
                  foreach (PrintPosItem positem in nband.Section.Components)
                  {
                    if (!positem.Hidden)
                    {
                      ...
                      Rectangle nrec = new Rectangle(Twips.TwipsToPixels(positem.PosX, FDrawScale),
                         Twips.TwipsToPixels(positem.PosY, FDrawScale), Twips.TwipsToPixels(positem.Width, FDrawScale),
                         Twips.TwipsToPixels(positem.Height, FDrawScale));
                      ...
                      switch (cname)
                      {
                        ...
                        case "TRPBARCODE":
                          ...
                          break;
                        case "TRPCHART":
                          DrawChart(grsec, nrec, (ChartItem)positem);
                          grsec.DrawRectangle(drawpen, nrec);
                          break;
                      }
...
//        grsection.DrawImage(bitselec,
    ...
    private void DrawChart(Graphics gr, Rectangle nrec, ChartItem textitem)
    {
        DrawChartObject(gr, nrec, textitem, "CHART - " + textitem.ValueExpression);
    }
        ...
        try
        {
          ...
          switch (dstyle)
          {
            case ImageDrawStyleType.Crop:
              int nwidth = (int)Math.Round(bitwidth / dpires * dpix * FDrawScale);
              int nheight = (int)Math.Round(bitheight / dpires * dpiy * FDrawScale);
              if (nwidth > abit.Width)
                nwidth = abit.Width;
              if (nheight > abit.Height)
                nheight = abit.Height;
              if (nwidth > arec.Width)
              {
                nwidth = arec.Width;
              }
              else
                destrec = new Rectangle(destrec.Left, destrec.Top, nwidth, destrec.Height);
              if (nheight > arec.Height)
              {
                nheight = arec.Height;
              }
              else
                destrec = new Rectangle(destrec.Left, destrec.Top, destrec.Width, nheight);
              srcrec = new Rectangle(0, 0, (int)Math.Round(nwidth / FDrawScale), (int)Math.Round(nheight / FDrawScale));
              imaitem.SelectionRectangle = destrec;
    ...
    private void DrawChartObject(Graphics gr, Rectangle nrec, ChartItem textitem, string ntext)
    {
    ...
    private Point AlignToGridPixelds(Point source, BandInfo binfo)
    {
        ...
        npoint = Twips.AlignToGridPixels(npoint,FReport.GridWidth,FReport.GridHeight,
            DrawScale);
        ...
        if (CapturedSelection)
        {
          ...
          if (CapturedMove)
          {
            ...
            if (destband != null)
            {
              ...
              int nposx = npositem.PosX + Twips.PixelsToTwips(offsetx, DrawScale);
              int nposy;
              if (destband != selectedposband)
              {
                nposy = offsety + nitem.SelectionRectangle.Top;
                nposy = Twips.PixelsToTwips(nposy, DrawScale);
              }
              else
              {
                nposy = npositem.PosY + Twips.PixelsToTwips(offsety, DrawScale);
              }
              if (FReport.GridEnabled)
              {
                Point npoint = Twips.AlignToGridTwips(new Point(nposx, nposy),
                    FReport.GridWidth, FReport.GridHeight);
                nposx = npoint.X;
                nposy = npoint.Y;
                offsetx = Twips.TwipsToPixels(nposx - npositem.PosX, FDrawScale);
                offsety = Twips.TwipsToPixels(nposy - npositem.PosY, FDrawScale);
              }
              nposx = Twips.TwipsToPixels(nposx,FDrawScale)+destband.BandPosX+parentcontrol.Left;
              nposy = Twips.TwipsToPixels(nposy, FDrawScale) + destband.BandPosY+destband.TitleHeight
                  +parentcontrol.Top;
    ...
    private void npicture_MouseUp(object sender, MouseEventArgs e)
    {
      ...
      if (CapturedBand != null)
      {
        ...
        if (RightBandCapture)
        {
          ...
          if (newposx < (CapturedBand.BandPosX + ncontrol.Left))
            newposx = CapturedBand.BandPosX + ncontrol.Left;
          CapturedBand.Section.Width = Twips.PixelsToTwips(
        ...
        else
        {
          ...
          PreviousBand.Section.Height = Twips.PixelsToTwips(
        ...
        if (CapturedSelection)
        {
          ...
          else
          {
            ...
            newitem.PosX = Twips.AlignToGridTwips(Twips.PixelsToTwips(leftpos-createband.BandPosX-ncontrol.Left, FDrawScale), FReport.GridWidth, FReport.GridHeight);
            newitem.PosY = Twips.AlignToGridTwips(Twips.PixelsToTwips(toppos-createband.BandPosY-createband.TitleHeight-ncontrol.Top,FDrawScale),FReport.GridWidth,FReport.GridHeight);
            newitem.Width = Twips.AlignToGridTwips(Twips.PixelsToTwips(widthpos,FDrawScale),FReport.GridWidth,FReport.GridHeight);
            newitem.Height = Twips.AlignToGridTwips(Twips.PixelsToTwips(heightpos,FDrawScale), FReport.GridWidth, FReport.GridHeight);
    ...
    private Rectangle SelSetBounds(Rectangle newvalue)
    {
        if ((SelectedItems.Count == 1) && (SelectedItemsBands.Count == 1))
        {
            ...
            PrintPosItem positem = (PrintPosItem)SelectedItems.Values[0];
            positem.PosX = Twips.PixelsToTwips(newleft, FDrawScale);
            positem.PosY = Twips.PixelsToTwips(newtop, FDrawScale);
            positem.Width = Twips.PixelsToTwips(newwidth, FDrawScale);
            positem.Height = Twips.PixelsToTwips(newheight, FDrawScale);
    ...
    private Rectangle SelGetBounds()
    {
      ...
      if (SelectedItems.Count == 1)
      {
        ...
        if (sitem is PrintPosItem)
        {
          ...
          nrec = new Rectangle(Twips.TwipsToPixels(positem.PosX, FDrawScale),
             Twips.TwipsToPixels(positem.PosY, FDrawScale), Twips.TwipsToPixels(positem.Width, FDrawScale),
             Twips.TwipsToPixels(positem.Height, FDrawScale));
        }
      }
    ...
    {       
      ...
      else
      {
          ...
          if (expression.Length > 0)
          {
              ...
              newitem.PosX = Twips.AlignToGridTwips(Twips.PixelsToTwips(leftpos, FDrawScale), FReport.GridWidth, FReport.GridHeight);
              newitem.PosY = Twips.AlignToGridTwips(Twips.PixelsToTwips(toppos, FDrawScale), FReport.GridWidth, FReport.GridHeight);
