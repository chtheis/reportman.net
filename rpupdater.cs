...
namespace Reportman.Drawing
{
...
#if PocketPC
  public delegate void SetLastWriteTimeEvent(string filename,DateTime datewrite);
  public delegate DateTime GetLastWriteTimeEvent(string filename);
#endif
  public class Updater
  {
...
#if PocketPC
    public SetLastWriteTimeEvent OnSetLastWriteTime;
    public GetLastWriteTimeEvent OnGetLastWriteTime;
#else
    ...
    public static DataTable GetModifiedFiles(DataTable olderfiles, DataTable files,string filesdir,bool copycontent)
    {
...
#if PocketPC
        // Recurse directories
        string[] directories = Directory.GetDirectories(Path.Combine(sourcedir, subdir));
        foreach (string ndir in directories)
        {
            FillFiles(xtable, sourcedir,System.IO.Path.Combine(subdir, ndir), copycontent);
        }
/*        string[] tempfilescontent = Directory.GetFiles(Path.Combine(sourcedir,subdir), "*.*");
        string[] nfilescontent = (string[])tempfilescontent.Clone();
        int i = 0;
        foreach (string afile in tempfilescontent)
        {
            nfilescontent[i] = Path.Combine(sourcedir,Path.Combine(subdir,System.IO.Path.GetFileName(afile)));
            i++;
        }*/
        string[] nfilescontent = Directory.GetFiles(Path.Combine(sourcedir, subdir), "*.*");
#else
        ...
        foreach (string fname in nfilescontent)
        {
            ...
            if (System.IO.Path.GetFileName(fname).ToUpper() != "THUMBS.DB")
            {
                ...
                if (OnGetLastWriteTime != null)
                    xrow["MODIFIED"] = OnGetLastWriteTime(fname);
                else
