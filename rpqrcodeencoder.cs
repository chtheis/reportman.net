...
namespace Reportman.Drawing
{
    public class QRCodeEncoder
    {
        ...
        {            
            ...
            apath = Path.Combine(Path.GetDirectoryName(apath),"qrcode");
            QRCODE_DATA_PATH = apath;
        }
		...
		public virtual bool[][] calQrcode(byte[] qrcodeData)
		{
			...
			try
			{
                MemoryStream bis = ReadResourceFile("qrv",qrcodeVersion,true,ec);
                
                ReadInput(bis, matrixX, 0, matrixX.Length);
                ReadInput(bis, matrixY, 0, matrixY.Length);
                ReadInput(bis, maskArray, 0, maskArray.Length);
                ReadInput(bis, formatInformationX2, 0, formatInformationX2.Length);
                ReadInput(bis, formatInformationY2, 0, formatInformationY2.Length);
                ReadInput(bis, rsEccCodewords, 0, rsEccCodewords.Length);
                ReadInput(bis, rsBlockOrderTemp, 0, rsBlockOrderTemp.Length);
			...
			try
			{
                MemoryStream bis = ReadResourceFile("qrvfr", qrcodeVersion, false, 0);
                ...
                ReadInput(bis, frameData, 0, frameData.Length);
                //bis.Close();
                //memoryStream.Close();
                //fis.Close();
			}
		...
		private static sbyte[] divideDataBy8Bits(int[] data, sbyte[] bits, int maxDataCodewords)
		{
            ...
            if (l2 > maxDataCodewords)
            {
                throw new Exception("Maximum barcode length:"+maxDataCodewords.ToString()+ " Found:"+l2.ToString());
            }
			for (int i = 0; i < l2; i++)
			{
				codewords[i] = 0;
        ...
        [ThreadStatic] 
        static SortedList<string, System.IO.MemoryStream> resourcefiles;
		private static sbyte[] calculateRSECC(sbyte[] codewords, sbyte rsEccCodewords, sbyte[] rsBlockOrder, int maxDataCodewords, int maxCodewords)
		{
			...
			try
			{
                MemoryStream bis = ReadResourceFile("rsc", rsEccCodewords, false,0);
                //String fileName = "rsc" + rsEccCodewords.ToString();
                //MemoryStream memoryStream = new MemoryStream(Resources.GetResource(fileName));                
                //BufferedStream bis = new BufferedStream(memoryStream);
				for (int i = 0; i < 256; i++)
				{
					ReadInput(bis, rsCalTableArray[i], 0, rsCalTableArray[i].Length);
				}
				//bis.Close();
                //memoryStream.Close();
			}
